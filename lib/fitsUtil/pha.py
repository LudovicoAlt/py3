...
class PHA:
    ...
    def doPrimary(self):
        ...
        hdr = pf.Header()
        hdr.update('Creator', 'osv.py V' + str(__version__), 'Software and version creating file')
        hdr.update('FILETYPE', 'SPECTRUM'              , 'Name for this type of FITS File')
    ...
    def doEvents(self):
        ...
        hdr.update('OBJECT'  , 'SimData'            , 'Object')
        hdr.update('RADECSYS', 'FK5'                , 'Stellar reference frame')
        hdr.update('EXPOSURE', self.exp             , 'exposure time (in seconds)')
        hdr.update('QUALITY', 0, '')
        hdr.update('EQUINOX' , 2000.0               , 'Equinox for RA and Dec')
        hdr.update('RA_OBJ'  , self.ra              , 'Calculated RA of burst')
        hdr.update('DEC_OBJ' , self.dec             , 'Calculated Dec of burst')
        hdr.update('ERR_RAD' , self.radErr          , 'Calculated Location Error Radius')
        ...
        hdr.update("HDUVERS", '1.2.0   ',           "Format version number")
        hdr.update("HDUVERS1", '1.1.0   ',           "Version of format (OGIP memo OGIP-92-007a)")
        ...
        hdr.update('HDUCLAS3', 'RATE'              , '')
        hdr.update('HDUCLAS4', 'TYPEI'             , '')
        hdr.update('EXTVER'  , 1                    , 'Version of this extension format')
